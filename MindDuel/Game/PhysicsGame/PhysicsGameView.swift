...
/// #16: solo physics round. Mirrors HistoryGameView/ScienceGameView.
struct PhysicsGameView: View {
    ...
    @State private var problem:               PhysicsProblem
    ...
    init(username: String, resumeRoomID: String? = nil) {
        ...
        let lvl = ProgressionStore.shared.physicsLevel
        _startLevel = State(initialValue: lvl)
        _problem    = State(initialValue: PhysicsProblemGenerator.generate(level: lvl))
    }
    ...
    var body: some View {
        ...
        .onAppear { restoreSavedSessionIfNeeded(); Task { try? await sessionService.startSession(mode: "physics", startPosition: startLevel) } }
    ...
    private func restoreSavedSessionIfNeeded() {
        ...
        problem    = PhysicsProblemGenerator.generate(level: progression.physicsLevel)
    ...
    private func saveSessionAndExit() {
        _ = MultiplayerStore.shared.saveStandaloneSoloPhysics(
    ...
    private var gameContent: some View {
        VStack(spacing: 0) {
            MDTopBar(title: String(localized: "mode_physics"),
        ...
        MDPrimaryCard {
            VStack(spacing: MDSpacing.xs) {
                Text(String(format: String(localized: "physics_level_problem"),
                            progression.physicsLevel, problemCount))
                    .mdStyle(.caption)
    ...
    private func handleAnswerTap(_ index: Int) {
        ...
        Task { try? await sessionService.submitAnswer(answeredAt: answeredAt, questionId: "physics-\(problemCount)", answer: problem.options[index], isCorrect: correct) }

        Task {
            try? await Task.sleep(nanoseconds: correct ? 250_000_000 : 300_000_000)
            if correct {
                totalAnswerTime += elapsedSeconds
                progression.recordCorrectAnswerTime(elapsedSeconds, mode: .physics)
                engine.recordCorrect()
                progression.advancePhysicsLevel()
            } else {
                progression.recordWrongAnswer(mode: .physics)
                engine.recordWrong()
            }
    ...
    private func finaliseRound(won: Bool) {
        ...
        roundResult = progression.applyPhysicsRound(
            correctCount: engine.correctCount,
            level: startLevel,
            avgTime: avgTime,
            won: won
    ...
    private func resetRound() {
        let lvl    = progression.physicsLevel
        startLevel = lvl
        problem    = PhysicsProblemGenerator.generate(level: lvl)
