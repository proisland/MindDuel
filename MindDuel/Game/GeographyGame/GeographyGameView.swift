...
struct GeographyGameView: View {
    ...
    var body: some View {
        ...
        .onAppear { restoreSavedSessionIfNeeded(); Task { try? await sessionService.startSession(mode: "geo", startPosition: startLevel) } }
    ...
    private func handleAnswerTap(_ index: Int) {
        ...
        Task { try? await sessionService.submitAnswer(answeredAt: answeredAt, questionId: "geo-\(problemCount)", answer: problem.options[index], isCorrect: correct) }
