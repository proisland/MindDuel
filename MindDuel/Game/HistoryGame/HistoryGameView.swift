...
struct HistoryGameView: View {
    ...
    var body: some View {
        ...
        .onAppear { restoreSavedSessionIfNeeded(); Task { try? await sessionService.startSession(mode: "history", startPosition: startLevel) } }
    ...
    private func saveSessionAndExit() {
        _ = MultiplayerStore.shared.saveStandaloneSoloHistory(
    ...
    private func handleAnswerTap(_ index: Int) {
        ...
        Task { try? await sessionService.submitAnswer(answeredAt: answeredAt, questionId: "history-\(problemCount)", answer: problem.options[index], isCorrect: correct) }
