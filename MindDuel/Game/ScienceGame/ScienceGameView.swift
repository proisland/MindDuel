...
/// #98: solo natural-science round. Mirrors BrainTrainingGameView so the
...
struct ScienceGameView: View {
    ...
    @State private var problem:               ScienceProblem
    ...
    private var gameContent: some View {
        VStack(spacing: 0) {
            MDTopBar(title: String(localized: "mode_science"),
        ...
        MDPrimaryCard {
            VStack(spacing: MDSpacing.xs) {
                Text(String(format: String(localized: "science_level_problem"),
                            progression.scienceLevel, problemCount))
                    .mdStyle(.caption)
                    .foregroundStyle(Color.mdText2)
                    .frame(maxWidth: .infinity, alignment: .center)
                Text(ScienceProblemGenerator.curriculumLabel(forLevel: progression.scienceLevel))
                    .mdStyle(.micro)
    ...
    private func handleAnswerTap(_ index: Int) {
        ...
        Task {
            ...
            if correct {
                ...
                progression.recordCorrectAnswerTime(elapsedSeconds, mode: .science)
                engine.recordCorrect()
                progression.advanceScienceLevel()
            } else {
                progression.recordWrongAnswer(mode: .science)
                engine.recordWrong()
            }
    ...
    private func resetRound() {
        let lvl    = progression.scienceLevel
        startLevel = lvl
        problem    = ScienceProblemGenerator.generate(level: lvl)
        problemCount      = 1
        elapsedSeconds    = 0
        totalAnswerTime   = 0
        feedbackIsCorrect = nil
