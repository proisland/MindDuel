...
struct SportGameView: View {
    ...
    var body: some View {
        ...
        .onAppear { restoreSavedSessionIfNeeded(); Task { try? await sessionService.startSession(mode: "sport", startPosition: startLevel) } }
    ...
    private func handleAnswerTap(_ index: Int) {
        ...
        Task { try? await sessionService.submitAnswer(answeredAt: answeredAt, questionId: "sport-\(problemCount)", answer: problem.options[index], isCorrect: correct) }
